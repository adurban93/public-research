...
def run_secondary_query(sb, query: str, lat: Optional[float] = None,
    ...
    """Type a follow-up query in the Maps search box, with URL fallback."""
    search_input = "#searchboxinput"
    
    try:
        element = sb.wait_for_element(search_input, timeout=14)
        ...
        logger.info(f"Secondary query executed: {query}")
        return
    except Exception as exc:
        logger.warning(f"Typing query failed ({exc}); reloading Maps for '{query}'")

    # Fallback: reload page with new query
    new_url = build_maps_search_url(query, lat=lat, lon=lon, zoom=zoom)
    
    logger.info(f"Reloading Maps with URL: {new_url}")
    sb.cdp.open(new_url)
    sb.sleep(wait_after)
    ensure_sidebar_feed(sb, timeout=14)
...
def run_scraper(config: Dict) -> Dict:
    ...
    with SB(**sb_kwargs) as sb:
        ...
        sb.cdp.open(url)
        sb.sleep(config.get("page_load_wait", 4.5))
